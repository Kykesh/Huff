...
(`.claude/agent-memory/frontend-lead/`) is your notebook; the DB is the system. Run these from the `trading_personal` root:
...
- **As you work:** `node scripts/gojo/agent-memory.mjs remember frontend-lead --kind {episodic|semantic|procedural} --scope {session|private|shared|global} --title "…" --body "…" --n <int> --source <path[:line]> --provenance {measured|assumed|ruled|quoted} [--lane L] [--files a,b]`
